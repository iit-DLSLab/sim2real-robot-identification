...
@configclass
class PegasusPaceCfg(PaceCfg):
    """Pace configuration for Pegasus robot."""
    robot_name: str = "pegasus_sim"
    data_dir: str = f"{ISAAC_ASSET_DIR}/../../datasets/pegasus/traj_0.pt"  # located in pace_sim2real/data/pegasus_sim/chirp_data.pt
...
@configclass
class PegasusPaceSceneCfg(PaceSim2realSceneCfg):
    """Configuration for Pegasus robot in Pace Sim2Real environment."""
    robot: ArticulationCfg = PEGASUS_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")

@configclass
class PegasusPaceEnvCfg(PaceSim2realEnvCfg):

    scene: PegasusPaceSceneCfg = PegasusPaceSceneCfg()
    sim2real: PaceCfg = PegasusPaceCfg()
